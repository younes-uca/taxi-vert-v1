package  ma.zsmart.taxi.ws.converter;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import ma.zsmart.taxi.zynerator.util.StringUtil;
import ma.zsmart.taxi.zynerator.converter.AbstractConverter;
import ma.zsmart.taxi.zynerator.util.DateUtil;
import ma.zsmart.taxi.bean.history.ThemeHistory;
import ma.zsmart.taxi.bean.core.Theme;
import ma.zsmart.taxi.ws.dto.ThemeDto;

@Component
public class ThemeConverter extends AbstractConverter<Theme, ThemeDto, ThemeHistory> {



    public  ThemeConverter(){
        super(Theme.class, ThemeDto.class, ThemeHistory.class);
    }

    @Override
    public Theme toItem(ThemeDto dto) {
        if (dto == null) {
            return null;
        } else {
        Theme item = new Theme();
            if(StringUtil.isNotEmpty(dto.getId()))
                item.setId(dto.getId());
            if(StringUtil.isNotEmpty(dto.getLibelle()))
                item.setLibelle(dto.getLibelle());
            if(StringUtil.isNotEmpty(dto.getCode()))
                item.setCode(dto.getCode());


        return item;
        }
    }

    @Override
    public ThemeDto toDto(Theme item) {
        if (item == null) {
            return null;
        } else {
    ThemeDto dto = new ThemeDto();
        if(StringUtil.isNotEmpty(item.getId()))
             dto.setId(item.getId());
        if(StringUtil.isNotEmpty(item.getLibelle()))
             dto.setLibelle(item.getLibelle());
        if(StringUtil.isNotEmpty(item.getCode()))
             dto.setCode(item.getCode());
        return dto;
        }
    }




}
