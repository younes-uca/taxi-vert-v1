...
export class AuthService {
    ...
    constructor(private http: HttpClient, private tokenService: TokenService, private router: Router) { }
       ...
       public loginChauffeur(username: string, password: string) {
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                this.error = null;
                const jwt = resp.headers.get('Authorization');
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/chauffeur']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
       ...
       public loginOperateur(username: string, password: string) {
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                this.error = null;
                const jwt = resp.headers.get('Authorization');
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/operateur']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
       ...
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/client']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
    ...
    public registerOperateur() {
        console.log(this.user);
        this.http.post<any>(this.API + 'api/users/save', this.user, {observe: 'response'}).subscribe(
            resp => {
                this.router.navigate(['operateur/login']);
            }, (error: HttpErrorResponse) => {
               console.log(error.error);
            }
        );
