...
@Component({
  selector: 'app-chauffeur-create-chauffeur',
  templateUrl: './chauffeur-create-chauffeur.component.html',
  styleUrls: ['./chauffeur-create-chauffeur.component.css']
})
export class ChauffeurCreateChauffeurComponent implements OnInit {
...
public saveWithShowOption(showList: boolean){
     this.chauffeurService.save().subscribe(chauffeur=>{
      if(chauffeur != null){
       this.chauffeurs.push({...chauffeur});
       this.createChauffeurDialog = false;
       this.submitted = false;
       this.selectedChauffeur = new ChauffeurVo();

    }else{
    this.messageService.add({severity: 'error', summary: 'Erreurs',detail: 'Chauffeur existe déjà' });
    }
...
this.errorMessages = new Array<string>();
       ...
       public async openCreateMarqueTelephone(marqueTelephone: string) {
          const isPermistted = await this.roleService.isPermitted('MarqueTelephone', 'add');
         if(isPermistted) {
         this.selectedMarqueTelephone = new MarqueTelephoneVo();
       ...
       public async openCreateMarqueVoiture(marqueVoiture: string) {
          const isPermistted = await this.roleService.isPermitted('MarqueVoiture', 'add');
         if(isPermistted) {
         this.selectedMarqueVoiture = new MarqueVoitureVo();
       ...
       public async openCreateModelApplication(modelApplication: string) {
          const isPermistted = await this.roleService.isPermitted('ModelApplication', 'add');
         if(isPermistted) {
         this.selectedModelApplication = new ModelApplicationVo();
       ...
       public async openCreateEtatChauffeur(etatChauffeur: string) {
          const isPermistted = await this.roleService.isPermitted('EtatChauffeur', 'add');
         if(isPermistted) {
         this.selectedEtatChauffeur = new EtatChauffeurVo();
